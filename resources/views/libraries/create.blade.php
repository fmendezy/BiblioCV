...
<div class="max-w-7xl mx-auto sm:px-6 lg:px-8 mt-6">
    <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-6">
        <div class="flex justify-between items-center mb-6">
            <h2 class="text-2xl font-semibold">{{ isset($library) ? 'Editar' : 'Nueva' }} Biblioteca</h2>
        </div>
        ...
        <form action="{{ isset($library) ? route('libraries.update', $library) : route('libraries.store') }}" method="POST" class="space-y-6">
            @csrf
            @if(isset($library))
                @method('PUT')
            @endif
            ...
            <div class="flex justify-end space-x-4">
                ...
                <button type="submit" class="inline-flex items-center px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-white hover:bg-gray-700 focus:outline-none focus:border-gray-900 active:bg-gray-900 transition duration-150 ease-in-out">
                    {{ isset($library) ? 'Actualizar' : 'Crear' }} Biblioteca
                </button>
            </div>
        </form>
    </div>
